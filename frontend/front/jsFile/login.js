...
function loginWithEcole42() {
    ...
    xhr.open('GET', serverIP +'api/get_client_id/', true);
    ...
    xhr.onload = function() {
        if (xhr.status >= 200 && xhr.status < 300) {
            ...
            const redirect_uri = serverIP; // Ecole 42 tarafından yetkilendirme/izinlendirme sonrası yönlendirileceğimiz URI
        ...
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                ...
                {  
                    ...
                    if(intra === true){
                        let prefixToRemove = serverIP + '/media/https%3A/';
...
function token(accessToken) { // accessToken (doğru kavram "yetki kodu" olmalı) parametresini kabul eder
    ...
    xhr.open('POST', serverIP + 'api/get_access_token/'); // Sunucunun /api/add "endpoint"ine bir POST istemi kurguluyoruz. Bu endpoint yüksek ihtimal yetki kodunu JWT ye takas edilmesi işlemini yönetecektir.
...
function loginup(data) {
    var xhr = new XMLHttpRequest();
    xhr.open('POST', serverIP + 'api/loginup/', true);
    xhr.setRequestHeader('Content-Type', 'application/json;charset=UTF-8');
