...
def loginup(request):
    if request.method == 'POST':
        ...
        user = ecole.objects.filter(username=username).first()

        if user and check_password(str(password), str(user.password)):
            logintrue(username)
            ...
            if not user:
                return JsonResponse({"error": "Kullanıcı bulunamadı"}, status=202)
            else:
...
def get_access_token(request):
    if request.method == 'POST':
        ...
        if authorization_code:
            ...
            client_id = os.getenv('Eclient_id')
            client_secret = os.getenv('Eclient_secret')
            redirect_uri = os.getenv('ServerIp')
...
def me(request):
    if request.method == 'POST':
        ...
        user_json = {
            ...
            'profileImage': os.getenv('ServerIp') + user.profile_image.url,
